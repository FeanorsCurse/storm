	...
	if (pl != "") {
		...
		switch (type) {
			case "flash":
				...
				setBool(pl, 'flash', 'allowfullscreen');//added to play flv files
			...
			case "flv": //added to play flv
				setBool(pl, 'flv', 'hiddengui');
				setBool(pl, 'flv', 'autostart');
				setBool(pl, 'flv', 'loop');
				setBool(pl, 'flv', 'showscalemodes');
				setBool(pl, 'flv', 'smoothvideo');
				setBool(pl, 'flv', 'jscallback');
				setStr(pl, 'flv', 'scalemode');
				setStr(pl, 'flv', 'buffer');
				setStr(pl, 'flv', 'startimage');
				setStr(pl, 'flv', 'starttime');
				setStr(pl, 'flv', 'defaultvolume');
				setStr(pl, 'flv', 'movie');
			break;
...
function getType(v) {
	...
	if (v.match(/(.+)(.*)\.flv/) || v.match(/(.+)(.*)\.mp4/)) {
		f.width.value = '328';
		f.height.value = '200';
		f.src.value = '../mediaplayer-5.2/player.swf?file=' + v; //FIXME: ohne ..?
		return 'flv'; //test
	}
...
function serializeParameters() {
	...
	switch (f.media_type.options[f.media_type.selectedIndex].value) {
		case "flash":
			...
			s += getBool('flash', 'allowfullscreen', false); //added to play flv video
		...
		case "flv":
			s += getBool('flv', 'hiddengui', false);
			s += getBool('flv', 'autostart', false);
			s += getBool('flv', 'loop', false);
			s += getBool('flv', 'showscalemodes', true);
			s += getBool('flv', 'smoothvideo', true);
			s += getBool('flv', 'jscallback', false);
			s += getStr('flv', 'scalemode');
			s += getStr('flv', 'buffer');
			s += getStr('flv', 'startimage');
			s += getStr('flv', 'starttime');
			s += getStr('flv', 'defaultvolume');
			s += getStr('flv', 'movie');
		break;
