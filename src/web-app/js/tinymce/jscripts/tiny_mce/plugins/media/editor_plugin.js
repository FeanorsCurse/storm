	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			function isMediaElm(n) {
				return /^(mceItemFlash|mceItemFlashVideo|mceItemShockWave|mceItemWindowsMedia|mceItemQuickTime|mceItemRealMedia)$/.test(n.className);
			};
			...
			ed.onInit.add(function() {
				...
				if (ed && ed.plugins.contextmenu) {
					ed.plugins.contextmenu.onContextMenu.add(function(th, m, e) {
						if (e.nodeName == 'IMG' && /mceItem(Flash|FlashVideo|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(e.className)) {
			...
			ed.onPostProcess.add(function(ed, o) {
				if (ed.getParam('media_use_script')) {
					o.content = o.content.replace(/<img[^>]+>/g, function(im) {
						...
						if (/^(mceTempFlash|mceTempFlashVideo|mceTempShockWave|mceTempWindowsMedia|mceTempQuickTime|mceTempRealMedia)$/.test(cl)) {
		...
		_objectsToSpans : function(ed, o) {
			...
			h = h.replace(/<script[^>]*>\s*write(Flash|FlashVideo|ShockWave|WindowsMedia|QuickTime|RealMedia)\(\{([^\)]*)\}\);\s*<\/script>/gi, function(a, b, c) {
				var o = t._parse(c);
		...
		_buildObj : function(o, n) {
			...
			if (p.src){
                ...
                // If '.flv' is in the source url, and '.swf' isn't
                if (p.src.indexOf('.flv') != -1 && p.src.indexOf('.swf') == -1){
                    p.src = this.url+'/img/player.swf?file=' + p.src;
                }
				
				// If '.ogg' is in the source url, and '.swf' isn't
                if (p.src.indexOf('.ogg') != -1 && p.src.indexOf('.swf') == -1){
                    p.src = this.url+'/img/FOggPlayer.swf?url=' + p.src + '&title=' + p.src;
                }
		...
		_spansToImgs : function(p) {
			...
			each(dom.select('span', p), function(n) {
				...
				if (dom.getAttrib(n, 'class') == 'mceItemObject') {
					...
					switch (ci) {//FIXME clsid für flv wichtig?
